	...
	public class InitializationStep : StepBase<InitializationStep>, IStep
	{
		...
		public void Run()
		{
			ExecuteSafe(() =>
			{
				...
				_store.SetOrAddItem(StoreKeys.ReportFullFileName, Path.Combine(_store.GetItem<DirectoryInfo>(StoreKeys.WorkPath).FullName, $"{reportTime:yyyy-MM-dd_HH_mm_ss}_BreakingChanges_Report.html"));
				_store.SetOrAddItem(StoreKeys.ReportObsoletesFullFileName, Path.Combine(_store.GetItem<DirectoryInfo>(StoreKeys.WorkPath).FullName, $"{reportTime:yyyy-MM-dd_HH_mm_ss}_Obsolete_Report.html"));
