...
namespace DetectPublicApiChanges.Analysis.SyntaxNodeAnalyzers
{
	...
	public class MethodSyntaxNodeAnalyzer : ISyntaxNodeAnalyzer
	{
		...
		public MethodSyntaxNodeAnalyzer(IIndexItemFactory indexItemFactory)
		{
		...
		public IIndexItem CreateItem(SyntaxNode syntaxNode)
		{
			...
			return _indexItemFactory.CreateItem(CreateKey(node), syntaxNode, Descriptor.AddObsoleteInformation(ObsoleteSyntaxNodeAnalyzer.GetObsoleteInformation(syntaxNode)).AddDescription($"The method {node.Identifier.ValueText} seems to be have been changed or removed"));
		}
