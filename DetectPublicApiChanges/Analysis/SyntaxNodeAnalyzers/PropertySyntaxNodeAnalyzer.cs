...
namespace DetectPublicApiChanges.Analysis.SyntaxNodeAnalyzers
{
	...
	public class PropertySyntaxNodeAnalyzer : ISyntaxNodeAnalyzer
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PropertySyntaxNodeAnalyzer"/> class.
		/// </summary>
		/// <param name="indexItemFactory">The index item factory.</param>
		public PropertySyntaxNodeAnalyzer(IIndexItemFactory indexItemFactory)
		{
		...
		public IIndexItem CreateItem(SyntaxNode syntaxNode)
		{
			...
			return _indexItemFactory.CreateItem(CreateKey(node), syntaxNode, Descriptor.AddObsoleteInformation(ObsoleteSyntaxNodeAnalyzer.GetObsoleteInformation(syntaxNode)).AddDescription($"The property {node.Identifier.ValueText} seems to be have been changed or removed"));
		}
