...
namespace DetectPublicApiChanges.Analysis.SyntaxNodeAnalyzers
{
	...
	public class GenericClassSyntaxNodeAnalyzer : ISyntaxNodeAnalyzer
	{
		...
		public IIndexItem CreateItem(SyntaxNode syntaxNode)
		{
			...
			return _indexItemFactory.CreateItem(CreateKey(node), syntaxNode, Descriptor.AddObsoleteInformation(ObsoleteSyntaxNodeAnalyzer.GetObsoleteInformation(syntaxNode)).AddDescription($"The generic class {node.Identifier.ValueText} seems to be have been changed or removed"));
		}
