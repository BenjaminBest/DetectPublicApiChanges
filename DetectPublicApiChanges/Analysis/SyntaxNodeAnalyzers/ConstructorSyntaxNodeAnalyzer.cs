...
namespace DetectPublicApiChanges.Analysis.SyntaxNodeAnalyzers
{
	...
	public class ConstructorSyntaxNodeAnalyzer : ISyntaxNodeAnalyzer
	{
		...
		public ConstructorSyntaxNodeAnalyzer(IIndexItemFactory indexItemFactory)
		{
		...
		public IIndexItem CreateItem(SyntaxNode syntaxNode)
		{
			...
			return _indexItemFactory.CreateItem(CreateKey(node), syntaxNode, Descriptor.AddObsoleteInformation(ObsoleteSyntaxNodeAnalyzer.GetObsoleteInformation(syntaxNode)).AddDescription($"The ctor {node.Identifier.ValueText} seems to be have been changed or removed"));
		}
